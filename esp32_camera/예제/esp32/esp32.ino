 ...
 *
 * MQTT 통신 규약 (Zero-Touch Media Control):
 *   - 토픽: zerotouch/cam01/face (소문자 고정, 와일드카드 없음)
 *   - 페이로드: {"detected":1,"face":2,"timestamp":123}
 *     detected = 1/0 (얼굴 감지 여부), face = 감지된 얼굴 개수
 *   - QoS 1, Retain true (재접속 시 마지막 상태 즉시 수신)
 *   - 상태가 바뀌면 즉시, 안 바뀌면 HEARTBEAT_MS 주기로 발행
 *   - 보드가 죽으면 브로커가 LWT {"detected":0,"face":0} 을 대신 발행
 */
...
const char* password    = "01044215154";
...
// ================= [2. MQTT] =================
void publishFaceState(bool detected, uint32_t face_count) {
    char payload[64];
    int n = snprintf(payload, sizeof(payload),
                     "{\"detected\":%d,\"face\":%lu}",
                     detected ? 1 : 0, (unsigned long)face_count,
...
void loop() {
    ...
    if (changed || (now_ms - last_publish_ms) >= HEARTBEAT_MS) {
        publishFaceState(face_detected, face_count);
    }
